...
module.exports = function (opt) {
  ...
  function createNonRetinaCanvas (retinaCanvas, cb) {
    var width = Math.ceil(retinaCanvas.width() / 2);
    var height = Math.ceil(retinaCanvas.height() / 2);
  ...
  function createStyle (layerInfo, sprite, retinaSprite) {
    ...
    if (retinaSprite) {
      ...
      lodash.forEach(sprites, function (sprite, i) {
        sprites[i].x = Math.ceil(sprite.x / 2);
        sprites[i].y = Math.ceil(sprite.y / 2);
        sprites[i].width = Math.ceil(sprite.width / 2);
        sprites[i].height = Math.ceil(sprite.height / 2);
      });
    }
